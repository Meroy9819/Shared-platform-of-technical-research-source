...
class changeSciAchiForm(forms.Form):
    error = (
        ('title', '标题错误'),
        ('author', '作者错误'),
        ('keyword', '关键词错误'),
    ...
    Reporttype = forms.ChoiceField(label='错误类型', choices=error)
    changeContent=forms.CharField(label="内容修改", max_length=2048,
                                    widget=forms.TextInput(attrs={'class': 'form-control'}))
...
class ChangeExpertForm(forms.Form):
    ...
    changeContent = forms.CharField(label="举报内容说明详情", max_length=6000,
                                    widget=forms.TextInput(attrs={'class': 'form-control'}))
    Reporttype = forms.ChoiceField(label='错误类型', choices=error)
#   captcha = CaptchaField(label='验证码')